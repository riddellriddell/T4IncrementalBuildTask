...
- Shares the standalone API's single source of truth: `TemplateCompiler.cs` and `FileScanUtility.cs` live in `T4CodeGen/` and are pulled into this library via linked-source `<Compile Include="..\T4CodeGen\..." Link="..."/>`. The same one copy compiles into both `CustomBuildTasks.dll` and the `T4CodeGen.exe` CLI (`T4CodeGen/Program.cs`); no duplication, no drift.

## Local Contracts

- `CustomBuildTasks.csproj` — classic C# class library, .NET Framework v4.7.2, AnyCPU. Output `bin\Debug\CustomBuildTasks.dll` is the exact path hardcoded by the root `RunCodeGen.targets`; do not move the output or switch to SDK-style layout without updating that reference. Compiles `..\T4CodeGen\TemplateCompiler.cs` and `..\T4CodeGen\FileScanUtility.cs` via linked-source `<Compile Include Link>` (single source of truth shared with the CLI exe). Engine/Roslyn assemblies are referenced via `<Reference><HintPath>` into the vendored `..\T4CodeGen\tools\<package>\<version>\` — do not convert back to `PackageReference`/`packages.config` (standalone build, no restore/network).
- `TemplateCompiler.cs` (lives at `T4CodeGen\TemplateCompiler.cs`, linked-source) — the standalone, MSBuild-independent generation pipeline (namespace `T4BuildTools`); the task is a thin adapter over it. Public static `TemplateCompiler.Compile(name, inputFiles, t4Templates, generatedFiles, baseIntermediateOutputPath, defaultFileOutputPath, Action<string> log)` takes plain `string`/`IList<string>` and returns a `TemplateCompilerResult` (`Success` + `TemplateFailures`); diagnostics flow through the `log` sink, failures through the result object. No `ITaskItem`, no `TaskLoggingHelper`, no `Microsoft.Build.*` references — host it from any front-end (task, CLI exe, test).
...
- `FileScanUtility.cs` (lives at `T4CodeGen\FileScanUtility.cs`, linked-source) — static helpers: `ScanFileWithRegex`, `ConvertMatchListToStringList` (group 1 only), `ConvertFileListToExistingFileList`, `ConvertFileListToChangedSinceFileList`.
