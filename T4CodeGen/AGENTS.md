...
- `Program.cs` — thin argument-to-API mapper only; it must not re-implement any pipeline logic (that lives in `TemplateCompiler.cs`). Parses the six task inputs (`Name`, `InputFiles`, `T4Templates`, `GeneratedFiles`, `BaseIntermediateOutputPath`, `DefaultFileOutputPath`) from CLI args or `@response.rsp`, calls `TemplateCompiler.Compile`, writes compiler log lines to stdout, forwards `TemplateFailures` to stderr verbatim (matching the task's `Log.LogError` text), and returns exit `0` on success / non-zero otherwise.
  - Flags are **case-insensitive**; help aliases are `-h`/`-help`/`-?`/`/?` — any one appearing anywhere (including inside a response file) prints usage to **stdout** and exits `0`, checked before parsing. Help tokens are matched exactly (case-sensitive).
  - Lists split on `|` if the value contains one, otherwise on `;`; empty entries dropped, entries trimmed and de-duplicated; repeated list flags accumulate (scalars last-write-wins). `-GeneratedFiles` is optional (invalidation input only). Both build-state artifacts are **literal concatenations** of `-BaseIntermediateOutputPath` with `GlobalFileManifest.T4Manifest` / `GeneratedFiles` (no separator inserted).
  - Response files: `@file` expands in place, left to right — each line trimmed, blank lines and `#`-comment lines skipped, one argument per line (no quote stripping, no space-splitting); a missing file writes `Response file not found: <path>` to stderr and exits `2` with no usage.
  - Exit codes: `0` success (nothing on stderr); `1` any template failed (per-template failure lines on stderr); `2` invalid invocation — zero args (usage → stdout), or missing response file / unknown flag / missing value / missing required argument (error + usage → stderr).
...
- `README.md` — the **normative integration contract** for `T4CodeGen.exe`: response-file grammar, argument semantics, exit codes, stdout/stderr channels, the `T4Gen_*` markers + the three template parameters, and a test-coverage mapping, plus build/usage and the worked example against the test bed. `Program.cs`/`TemplateCompiler.cs` are the reference implementation the contract must match; where the README and code disagree, correct the README to the code.
