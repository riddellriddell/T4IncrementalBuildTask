...
- `implementation plans/` — concrete, sequenced implementation plans (tier-4), derived from the design/goals; only implementation plans live in this subfolder. Current set (Goal 1.1, one plan per deliverable, see `goals1.md`):
  ...
  - Vendoring/consumption plan set (**Draft 2026-09-07**; `FEATURE_self-contained-t4codegen-project.md` and `FEATURE_relocate-tools-under-t4codegen.md` landed the same day), targeting making the standalone `T4CodeGen.exe` easy to vendor into other projects (see `T4CodeGen/README.md`, root `AGENTS.md`):
