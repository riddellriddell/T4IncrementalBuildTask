...
- `CustomBuildTasks/` — C# build-task library driven by the thin MSBuild task adapter `BuildT4TextFiles.cs`; the standalone, MSBuild-independent `TemplateCompiler` pipeline and `FileScanUtility.cs` live in `T4CodeGen/` (single source of truth) and are pulled in here via linked-source `<Compile Include Link>`. Also hosts `AddMatchingFilesToOutput.cs`, `CustomBuildTasks.csproj`, `Debug.testproj`. See `CustomBuildTasks/AGENTS.md`.
...
- `T4CodeGen/` — self-contained console CLI front-end over the standalone `TemplateCompiler` API: one `.csproj`, no `ProjectReference` to `CustomBuildTasks`. Owns `TemplateCompiler.cs`/`FileScanUtility.cs` as the single source of truth (linked-compiled into `CustomBuildTasks`); `Program.cs` is a second front-end beside the MSBuild task that runs the same pipeline from the command line (`README.md` has usage). Also owns the vendored `tools/` subtree — `T4CodeGen/tools/` — the single copy of the engine/compiler/runtime assemblies both the CLI and the MSBuild task consume via HintPath (see `T4CodeGen/AGENTS.md`). The CLI's copy/deployment boundary is this one folder. See `T4CodeGen/AGENTS.md`, `agents/plans/design.md`, and `T4CodeGen/README.md`.
