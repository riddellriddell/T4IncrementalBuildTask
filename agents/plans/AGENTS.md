...
- `implementation plans/` — concrete, sequenced implementation plans (tier-4), derived from the design/goals; only implementation plans live in this subfolder. Current set (Goal 1.1, one plan per deliverable, see `goals1.md`):
  ...
  - Vendoring/consumption plan set (**Draft 2026-09-07**; `FEATURE_self-contained-t4codegen-project.md`, `FEATURE_relocate-tools-under-t4codegen.md`, `FEATURE_exe-integration-contract.md`, and `RELEASE_prebuilt-artifact-github-releases.md` landed the same day, the last with its tag-push publish still pending), targeting making the standalone `T4CodeGen.exe` easy to vendor into other projects (see `T4CodeGen/README.md`, root `AGENTS.md`):
    - `RELEASE_prebuilt-artifact-github-releases.md` — publish a self-contained release zip (`T4CodeGen.exe` + the 9-DLL runtime-referenced engine/Roslyn subset) via GitHub Releases with semver tags, so consumers fetch a prebuilt asset instead of vendoring source. **Landed 2026-09-07** (`scripts\package-release.ps1` + `.github\workflows\release.yml`; local packaging + standalone-zip verification passed — pending pushing tag `v1.0.0` to publish the first GitHub Release). **Highest payoff.**
    - `FEATURE_self-contained-t4codegen-project.md` — `T4CodeGen/` is now one self-contained csproj: `TemplateCompiler.cs` + `FileScanUtility.cs` live in the project (dropping the `ProjectReference` to `CustomBuildTasks`), and `CustomBuildTasks.csproj` compiles them via `<Compile Include Link>` so the task DLL is unchanged. **Landed 2026-09-07.**
    - `FEATURE_relocate-tools-under-t4codegen.md` — physically move the vendored 12-package `tools/` tree to `T4CodeGen/tools/` and rewire both csproj HintPaths, so the exe's drop-in boundary is project-level not repo-root-level (`.gitignore` whitelist updated first). **Landed 2026-09-07.**
    ...
    - `FEATURE_exe-integration-contract.md` — write an explicit integration contract for `T4CodeGen.exe` in `T4CodeGen/README.md`: response-file grammar, arg semantics, exit codes, stdout/stderr channels, and the `T4Gen_*` output markers (pinned to the actual `Program.cs`/`TemplateCompiler.cs` behavior). **Moderate payoff.** **Landed 2026-09-07.**
