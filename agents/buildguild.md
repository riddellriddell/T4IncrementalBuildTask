...
- No `t4` on PATH, no NuGet restore, no network: the engine + Roslyn runtime assemblies are vendored under `T4CodeGen\tools\` and referenced by HintPath.
...
- `T4CodeGen\tools\` is deliberately committed (vendored engine/Roslyn/runtime assemblies). Keep them tracked; they are the standalone build's only dependency copies.
