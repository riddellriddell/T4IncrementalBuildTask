...
- `BuildT4TextFiles.cs` — the main task. Required parameters: `Name`, `InputFiles` (seed sources), `T4Templates`, `GeneratedFiles`, `BaseIntermediateOutputPath`, `DefaultFileOutputPath`.
  ...
  - Failure semantics (per template): a failing template removes its partial outputs from the temp `GeneratedFiles` folder, logs an MSBuild error naming the template, and continues to the next template; `Execute()` returns `false` if any template failed (successful templates still copy their outputs). An exception escaping the engine host is treated as a template failure.
