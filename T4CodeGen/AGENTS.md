...
- `msbuild T4CodeGen\T4CodeGen.csproj` (offline) — must produce `T4CodeGen\bin\Debug\T4CodeGen.exe` with the engine/Roslyn runtime DLLs copied beside it from `tools\` only, and **no** `CustomBuildTasks.dll` in `bin\Debug`.
