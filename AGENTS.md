...
- `T4CodeGen/` — self-contained console CLI front-end over the standalone `TemplateCompiler` API: one `.csproj`, no `ProjectReference` to `CustomBuildTasks`. Owns `TemplateCompiler.cs`/`FileScanUtility.cs` as the single source of truth (linked-compiled into `CustomBuildTasks`); `Program.cs` is a second front-end beside the MSBuild task that runs the same pipeline from the command line (`README.md` has usage). See `T4CodeGen/AGENTS.md`, `agents/plans/design.md`, and `T4CodeGen/README.md`.
