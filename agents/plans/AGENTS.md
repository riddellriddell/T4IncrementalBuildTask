...
- `implementation plans/` — concrete, sequenced implementation plans (tier-4), derived from the design/goals; only implementation plans live in this subfolder. Current set (Goal 1.1, one plan per deliverable, see `goals1.md`):
  - `GOAL_1_1_host-engine-in-process.md` — Deliverable 1: swap the `t4.exe`/`powershell.exe` shell-out for an in-process Mono.TextTemplating engine (+ Roslyn), keeping parameters, markers, and copy semantics.
  - `GOAL_1_1_rename-change-file-mainfest-parameter.md` — Deliverable 2: rename `ChangeFileMainfest` -> `ChangeFileManifest` in task and templates. Recommended to land before the engine swap.
